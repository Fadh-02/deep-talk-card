...
function updateCardUI(q) {
    ...
    document.getElementById('badge-text').innerText = q.category.toUpperCase();
    
    const badge = document.getElementById('card-badge');
    // Ensure you have these colors defined in style.css or use inline style
    // Clear previous colors and apply new one
    badge.style.backgroundColor = getCategoryColor(q.category);
    
    // Update Star Icon
    const isFav = favorites.some(f => f.id === q.id);
    const starIcon = document.getElementById('star-icon');
    if (isFav) {
        starIcon.style.fill = "#facc15";
...
function getCategoryColor(cat) {
    switch(cat) {
        case 'Pasangan': return '#ec4899'; // Pink
        case 'Kawan': return '#3b82f6';    // Blue
        case 'Deep': return '#8b5cf6';     // Purple
...
function renderFavorites() {
    ...
    if (favorites.length === 0) {
        container.innerHTML = `<p style="text-align:center; color:#94a3b8; padding:40px;">Tiada soalan kegemaran.</p>`;
        return;
    }
    container.innerHTML = favorites.map(q => `
        <div class="list-item" style="background:#1e293b; padding:15px; border-radius:15px; margin-bottom:10px; display:flex; justify-content:space-between; align-items:center;">
            <div>
                <small style="color:${getCategoryColor(q.category)}; font-weight:bold;">${q.category.toUpperCase()}</small>
                <p style="color:white; margin-top:4px;">${q.text}</p>
            </div>
            <i data-lucide="star" style="fill:#facc15; color:#facc15; shrink:0;"></i>
        </div>
    `).join('');
    lucide.createIcons();
}
...
function renderHistory() {
    ...
    if (history.length === 0) {
        container.innerHTML = `<p style="text-align:center; color:#94a3b8; padding:40px;">Belum ada sejarah.</p>`;
        return;
    }
    container.innerHTML = history.map(q => `
        <div class="list-item" style="background:#1e293b; padding:15px; border-radius:15px; margin-bottom:10px; border-left: 4px solid ${getCategoryColor(q.category)}">
            <div>
                <small style="color:#94a3b8;">${q.category}</small>
                <p style="color:#cbd5e1; margin-top:4px; font-size:0.9rem;">${q.text}</p>
            </div>
        </div>
    `).join('');
    lucide.createIcons();
